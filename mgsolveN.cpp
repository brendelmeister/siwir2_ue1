...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        ...
        cout<<"l: number of levels; n: number of V-cycles"<<endl;
        exit(EXIT_FAILURE);
    }
    l = atoi(argv[1]); // set levels
    n = atoi(argv[2]); // set V-cycles
    NX = NY = getGridPointsNeumann(); // set number of grid points
    H = 1.0/(NX); // set cell size


    double* u = new double[NX*NY]; // initialize arrays
    memset(u,0,sizeof(double)*NY*NX);
    initializeGrid(u);
    double* f = new double[NX*NY]; // right hand side
     ...
     */
    for(int i=0;i<n;i++){ //multi-grid steps
        mgm(u,f,2,1,NX, NY);
    }
...
int getGridPointsNeumann(){
    return pow(2,l)+2; // plus two for the ghost layer
}

void initializeGrid(double* u){
   
   
   // 1 to NX-2 because of ghost layers
    for(int i = 1; i < NX-2; ++i){ 
    // boundary condition: x*(1-x)
        // top row; y = 1
        u[(NY-2) * NX + i] = (i*H-0.5*H)*(1-(i*H-0.5*H));
        // bottom row; y = 0
        u[NX+i] = (i*H-0.5*H)*(1-(i*H-0.5*H));
...
void restriction(double* f_co,double* res,const int n_x,const int n_y){
    ...
    // x=0(left boundary) and x=1(right boundary)
    ...
    // y=0(bottom boundary) and y=1(top boundary)
